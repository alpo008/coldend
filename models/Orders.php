        ...
        if (parent::beforeSave($insert)){
            if (!!$this->incomeTo){
                foreach ($this->lists as $list){
                    $newIncomsRecord = new Incoms();
                    $newIncomsRecord->materials_id = $list->materials_id;
                    $newIncomsRecord->qty = $list->qty;
                    $newIncomsRecord->came_from = 0;
                    $newIncomsRecord->came_to = (int) $this->incomeTo - 1;
                    $newIncomsRecord->responsible = (!!Yii::$app->user->identity) ? Yii::$app->user->identity->name . ' ' . Yii::$app->user->identity->surname : Yii::t('app', 'Generated by system');
                    $newIncomsRecord->trans_date = date('Y-m-d');
                    $newIncomsRecord->ref_doc = $this->ref_doc;
                    $newIncomsRecord->comment = Yii::t('app', 'Generated by system');
                    $newIncomsRecord->save();
                }
            }
        return true;
        }else{
